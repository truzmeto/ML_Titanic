...
cross validation that helps to choose best hyper parameter for an optimum prediction. Then, I combine all 
...
As depicted above, the best number for nearest neighbor is `r knnFit$bestTune` that gave highest
cross validation accuracy `r max(knnFit$results[2])`. Algorithm automatically chooses best fit for further processing.      
...
Boosting is an ensemble method that combines a lot of weak learners to create strong learner [link to good tutorial](https://gormanalysis.com/gradient-boosting-explained/). 
...
```{r nnet, message=FALSE, warning=FALSE}
...
## Fit Radial Kernel----------------------------------------------------------
SVMgridRad <- expand.grid(C = (1:30)*0.1 + 0.2, sigma = c(0.01,0.02,0.03))
...
write.csv(output, file = 'rf_prediction.csv', row.names = F)
```
