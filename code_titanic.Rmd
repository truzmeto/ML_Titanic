...
knn_acc <- max(knnFit$results$Accuracy)
```
...
train_pred_trees <- predict(treeFit, Training, type = "class")
...
TrainPredDF <- data.frame("knn" = as.numeric(train_pred_knn) -1,
                       "trees" = as.numeric(train_pred_trees) -1,
...
write.csv(output, file = 'majoroty_prediction.csv', row.names = F)
```
